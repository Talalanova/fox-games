<template>
  <div class="item-page item" v-if="productReady">
    ...
    <Breadcrumbs>
      ...
      <router-link
        v-if="itemData.ancestor_slug_3"
        :to="'/Category/' + itemData.ancestor_slug_3"
        >{{ itemData.ancestor_3 }}</router-link
      >
      <router-link
        v-if="itemData.ancestor_slug_2"
        :to="'/Category/' + itemData.ancestor_slug_2"
        >{{ itemData.ancestor_2 }}</router-link
      >
      <router-link :to="mainSlug">{{ itemData.ancestor }}</router-link>
      <a>{{ itemData.title }}</a>
    </Breadcrumbs>
    <div class="item__data">
      <div
        class="item__current-pic"
        v-bind:class="[getStatus(itemData.inStock)]"
        :title="stockStatus[itemData.inStock]"
      >
        <span class="prev" @click="updateImage(selectedPic - 1)"></span>
        <span class="next" @click="updateImage(selectedPic + 1)"></span>
        <img
          :src="itemData.pics[selectedPic]"
          width="560"
          height="370"
          :alt="itemData.title"
        />
      ...
      <div class="item__thumbs">
        <img
          v-for="(item, index) in itemData.pics.slice(0, 4)"
          :key="item.index"
          :src="item"
      ...
      <div class="item__information">
        <span
          :class="[itemData.discont > 0 ? 'price price--discont' : 'price']"
        >
        ...
        <span
          v-for="(value, name) in itemParameters"
          v-bind:key="value.index"
          :class="[itemData[name] == null ? 'row' : 'row row--underline']"
        >
          <span v-if="itemData[name] !== null" class="parameters">{{
            value
          }}</span>
          <span v-if="itemData[name] !== null" class="value">
            {{ itemData[name] }}</span
          >
        </span>
        <router-link to="/delivery" class="delivery"
          >Условия заказа</router-link
        >
        <button
          @click="addToCart(itemData)"
          type="button"
          :disabled="itemData.amount == 0"
          class="item__button"
        >
          <svg
            width="18"
    ...
    <div id="tabs" class="container">
      <div class="tabs">
        ...
        >
        <a
          v-if="accessorys.length"
          v-on:click="activetab = '4'"
          v-bind:class="[activetab === '4' ? 'active' : '']"
          >Аксессуары</a
        >
        <a
          v-if="itemData.comments"
          v-on:click="activetab = '5'"
          v-bind:class="[activetab === '5' ? 'active' : '']"
          >Отзывы и вопросы({{ itemData.comments.length }})</a
        >
      </div>
      <div class="content">
        <div
          v-if="activetab === '1'"
          class="tabcontent text-hidden"
          ref="tabContent"
        ...
        <div
          ...
          class="tabcontent text-hidden"
          ref="tabContent"
          v-html="itemData.rules"
        ></div>
        <div
          v-if="activetab === '3'"
          class="tabcontent section__content section__content--flex"
        >
          <ItemCard
        ...
        <div v-if="activetab === '5'" class="tabcontent">
          <div>
            <Comment
              v-for="item in itemData.comments.slice(0, 3)"
            ...
            <form method="post" name="comments">
              <input
                class="comment__field--name"
                type="text"
                :placeholder="getRandomArrayElement(someRandomNames).name"
                v-model="commentName"
                name="name"
              />
              <textarea
              ...
              <button
                type="button"
                class="tabcontent__button--inverted"
    ...
    <MainSection
      ...
      link="/newsales"
      class="item__recommended section--flex"
    >
      <ItemCard
...
export default {
  ...
  methods: {
    ...
    updateComments() {
      this.itemData.comments = this.itemData.comments.slice(
        3,
        this.itemData.comments.length + 1
      );
    },
    addComment() {
      let newComment = {
      ...
      fetch(
        "http://api.foxhole.club/api/product/" +
        ...
        fetch("http://api.foxhole.club/api/product/" + this.$route.params.id)
          ...
          .then((json) => {
            ...
            element.messages.forEach((item, index) => {
              element.messages[index].created_at = new Date(
                item.created_at
            ...
            if (this.collection_id == 0) {
              ...
              fetch(
                "http://api.foxhole.club/api/product/filter?is_accessory=0&group_id=" +
                  this.collection_id
              )
                .then((response) => {
                  if (response.ok) {
            ...
            fetch(
              "http://api.foxhole.club/api/product/filter?tags=[" +
                this.tags +
                "]"
            )
              .then((response) => {
            ...
            ancestor = element.ancestor.title;
            ancestor_slug = element.ancestor.slug;

            if (element.ancestor.ancestor !== null) {
              ancestor_2 = element.ancestor.ancestor.title;
              ancestor_slug_2 = element.ancestor.ancestor.slug;

              if (element.ancestor.ancestor.ancestor !== null) {
                ancestor_3 = element.ancestor.ancestor.ancestor.title;
                ancestor_slug_3 = element.ancestor.ancestor.ancestor.slug;
              }
            }
            ...
            this.itemData = {
              ...
              ancestor_2: ancestor_2,
              ancestor_slug_2: ancestor_slug_2,
              ancestor_3: ancestor_3,
              ancestor_slug_3: ancestor_slug_3,
            };
            if (ancestor_3 == "") {
              this.mainSlug = "/Category/" + this.itemData.ancestor_slug;
            } else {
              this.mainSlug =
                "/Category/" +
                this.itemData.ancestor_slug_2 +
                "/" +
