    ...
    <span class="header__search-bar">
      <div class="form">
        ...
        <div class="header__datalist datalist" v-if="searched.length">
          <router-link
            :to="'/item-full/' + item.slug + '/prd/' + item.id"
            v-for="item in searched.slice(0, 15)"
            :key="item"
            class="datalist__row"
            @click="this.searched = []"
          >
      ...
      <router-link
        to="/cart"
        :class="[
          CART.length ? 'header__cart header__cart--active' : 'header__cart',
        ]"
      >
        <svg
          width="25"
